...
# Verificación de la disponibilidad de LBPHFaceRecognizer
if not hasattr(cv2.face, 'LBPHFaceRecognizer_create'):
    print("Error: LBPHFaceRecognizer no está disponible en tu instalación de OpenCV.")
    exit()

# Creación del reconocedor de caras LBPH con parámetros ajustados
...
# Verificación del tamaño del modelo
model_size = os.path.getsize('modeloLBPHFace.xml')
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        try:
            # Predicción del rostro usando el modelo LBPH
        ...
        #if accuracy >= 30 and label < len(imagePaths):
